...
export class VideoPlayer {
  ...
  public pipWindow?: PictureInPictureWindow;
  ...
  constructor(
    private readonly _mode: 'embedded' | 'fullscreen',
    private readonly _url: string,
    private readonly _playerId: string,
    private readonly _videoRate: number,
    private readonly _videoExitOnEnd: boolean,
  ...
  private _setupSourceAndListeners(): Promise<boolean> {
    return new Promise((resolve) => {
      if (!this.videoEl) return resolve(false);
      ...
      // PIP listeners
      this.videoEl.addEventListener('enterpictureinpicture', (e: any) => {
  ...
  private _onEnded() {
    ...
    if (this._videoExitOnEnd) {
      if (this._mode === 'fullscreen') this._exitFullscreen();
  ...
  private _onReady() {
    if (!this._firstReadyToPlay) return;
    this._createEvent('Ready', this._playerId);
    this.videoEl.muted = false;
    if (this._mode === 'fullscreen') this.videoEl.play().catch(() => {});
  ...
  private _createEvent(ev: string, playerId: string, msg?: string) {
    const detail = msg
      ...
      : { fromPlayerId: playerId, currentTime: this.videoEl?.currentTime ?? 0 };
